...
public:
  ...
  const ServerSocketInterface &
  operator=(const ServerSocketInterface &) = delete;
...
public:
  template <typename... Targs> ServerSocketMux(Targs &...Fargs) {
    add(Fargs...);
  }
...
protected:
  template <typename Server> void add(Server &server) {
...
public:
  ...
  public:
    ...
    virtual const char *
    get_subscription_pattern(SubscriptionId id) const override;
  ...
  Server(uint16_t port = 1883) : Server(new ServerSocket<::WiFiServer>(port)) {
    TRACE_FUNCTION
  }
...
protected:
  ...
  virtual ConnectReturnCode auth(const char *client_id, const char *username,
                                 const char *password) {
    return CRC_ACCEPTED;
  }

...
public:
  ...
  public:
    Publish(Publisher &publisher, const PrintMux &print, const char *topic,
  ...
  virtual bool publish(const char *topic, const void *payload,
    ...
    TRACE_FUNCTION
    auto packet =
        begin_publish(get_c_str(topic), payload_size, qos, retain, message_id);
    packet.write((const uint8_t *)payload, payload_size);
    return packet.send();
  }

  bool publish(const String &topic, const void *payload,
               const size_t payload_size, uint8_t qos = 0, bool retain = false,
               uint16_t message_id = 0) {
    TRACE_FUNCTION
  ...
  bool publish_P(TopicStringType topic, PGM_P payload, uint8_t qos = 0,
                 bool retain = false, uint16_t message_id = 0) {
    TRACE_FUNCTION
    return publish_P(get_c_str(topic), payload, strlen_P(payload), qos, retain,
                     message_id);
  }

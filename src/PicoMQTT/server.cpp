...
Server::IncomingPublish::IncomingPublish(IncomingPacket &packet,
                                         Publish &publish)
    : IncomingPacket(std::move(packet)), publish(publish){TRACE_FUNCTION}
...
Server::Server(std::unique_ptr<ServerSocketInterface> server)
    : keep_alive_tolerance_millis(10 * 1000), socket_timeout_millis(5 * 1000),
...
bool ServerLocalSubscribe::publish_P(const char *topic, PGM_P payload,
  ...
  TRACE_FUNCTION
  const bool ret =
      Server::publish_P(topic, payload, payload_size, qos, retain, message_id);

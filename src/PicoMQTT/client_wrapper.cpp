...
int ClientWrapper::available_wait(unsigned long timeout) {
    ...
    while (true) {
        ...
        if (!connected()) {
            // A disconnected client might still have unread data waiting in
            // buffers. Don't move this check earlier.
            return 0;
        }
...
int ClientWrapper::read(uint8_t * buf, size_t size) {
    ...
    while (ret < size) {
        ...
        const unsigned long remaining_millis =
            socket_timeout_millis - elapsed_millis;
        ...
        const int chunk_size = size - ret < (size_t)available_size
                                   ? size - ret
...
ClientWrapper::operator bool() { return bool(client); }
